...
/*			Account account = new Account();
		...
		//Query query = session.createQuery("SELECT m FROM Market m where m.id = ?");
		//Query query = session.createQuery("SELECT m FROM Market m where m.id = :id");
			
		//System.out.println("Please enter the market id=");
		//query.setParameter(0, new Long(12));
		//query.setParameter("id", new Long(12));
